...
object SparkTweets {
  ...
  def main(args: Array[String]) {
    ...
    val tweet_file   = Paths.get("test/tweet_json.json").toAbsolutePath.normalize
    ...
    geo_tweets.take(10).foreach(t => {
      println(t.coordinates);
      println(t.entities.get_hashtags)
      println(t.coordinates.get.point)
    })
